...
Our final dataset comprises 933 labeled samples distributed across 6 NASA ER-2 flight campaigns:

\begin{center}
\begin{tabular}{lcccc}
\toprule
\textbf{Flight ID} & \textbf{Samples} & \textbf{Mean CBH (m)} & \textbf{Std CBH (m)} & \textbf{Date} \\
\midrule
F1 & 198 & 1247 & 412 & 2023-03-15 \\
F2 & 176 & 1589 & 523 & 2023-03-22 \\
F3 & 184 & 1103 & 387 & 2023-04-10 \\
F4 & 127 & 2341 & 891 & 2023-05-18 \\
F5 & 132 & 978 & 298 & 2023-06-05 \\
F6 & 116 & 1456 & 445 & 2023-06-19 \\
\midrule
\textbf{Total} & \textbf{933} & \textbf{1402} & \textbf{618} & --- \\
\bottomrule
\end{tabular}
\end{center}

Cloud base heights range from 234 m to 4521 m, with median 1289 m. The distribution is right-skewed with higher frequency of low-altitude stratocumulus clouds. Flight F4 exhibits substantially higher mean CBH and variance, representing a distinct meteorological regime.
...
\begin{enumerate}[leftmargin=*]
    \item Select target flight (F4, highest domain shift)
    \item Train baseline model on remaining 5 flights
    \item Sample $k \in \{5, 10, 20\}$ examples from F4
    \item Fine-tune baseline model on $k$ samples
    \item Evaluate on held-out F4 test set
...
Table~\ref{tab:model_comparison} presents the main validation results. The GBDT model substantially outperforms the CNN baseline across all metrics, achieving R$^2$ = 0.744 compared to 0.351 for the CNN. Mean absolute error for GBDT (117.4 m) is nearly half that of the CNN (236.8 m). Figure~\ref{fig:model_comparison} visualizes the performance comparison across all models.
...
ImageNet pre-training provides marginal improvement over the simple CNN (R$^2$ = 0.389 vs 0.351), suggesting limited transfer learning benefit from natural images to atmospheric imagery---consistent with findings by \citet{Neumann2019} for satellite data.
...
Per-flight performance (Figure~\ref{fig:per_flight}) reveals systematic differences:

\begin{center}
\begin{tabular}{lccc}
\toprule
\textbf{Flight} & \textbf{GBDT R$^2$} & \textbf{CNN R$^2$} & \textbf{CBH Variance} \\
\midrule
F1 & 0.812 & 0.423 & Low \\
F2 & 0.776 & 0.391 & Medium \\
F3 & 0.789 & 0.405 & Low \\
F4 & 0.521 & 0.187 & \textbf{High} \\
F5 & 0.851 & 0.462 & Low \\
F6 & 0.793 & 0.408 & Medium \\
\bottomrule
\end{tabular}
\end{center}

Flight F4 shows degraded performance for both models, corresponding to its high CBH variance (891 m std) and distinct meteorological regime. Residual analysis reveals larger errors for CBH > 2500 m, where training data are sparse (only 12\% of samples).

The CNN shows higher variance across folds (R$^2$ std = 0.075) compared to GBDT (std = 0.037), indicating less stable learning in the small-sample regime.

\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{../results/cbh/figures/paper/figure_per_flight_performance.png}
\caption{Per-flight performance showing systematic differences across meteorological regimes. Flight F4 exhibits substantially degraded performance for both GBDT and CNN models.}
\label{fig:per_flight}
\end{figure}
...
Leave-one-flight-out (LOFO) validation reveals severe domain shift:

\begin{center}
\begin{tabular}{lcc}
\toprule
\textbf{Test Flight} & \textbf{LOFO R$^2$} & \textbf{Within-CV R$^2$} \\
\midrule
F1 & 0.623 & 0.812 \\
F2 & 0.558 & 0.776 \\
F3 & 0.601 & 0.789 \\
...
Flight F4 shows catastrophic failure (R$^2$ = -0.98) when excluded from training, indicating strong distribution shift. Other flights show moderate degradation (0.1--0.2 R$^2$ drop).

Few-shot learning experiments on F4 (Figure~\ref{fig:few_shot}) show:
...
Even with 20 labeled F4 samples, performance remains far below within-distribution accuracy, suggesting fundamental distributional differences require investigation (e.g., different cloud types, extreme atmospheric conditions).

\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{../results/cbh/figures/domain_adaptation/few_shot_learning_curve.png}
\caption{Few-shot learning curves for Flight F4 domain adaptation. Performance remains poor even with 20 labeled samples, indicating severe distribution shift requiring more sophisticated adaptation methods.}
\label{fig:few_shot}
\end{figure}
...
The catastrophic failure on Flight F4 (R$^2$ = -0.98 in LOFO validation) highlights the challenge of cross-domain generalization. Analysis of F4 characteristics reveals:

\begin{itemize}[leftmargin=*]
    \item Substantially higher mean CBH (2341 m vs 1402 m overall)
    \item Larger CBH variance (891 m std vs 618 m overall)
    \item Different cloud types (likely mid-level altocumulus vs low stratocumulus)
    \item Different season (May vs March--June for other flights)
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Root-cause analysis:} Investigate why F4 fails (feature distribution analysis, covariate shift decomposition).
