...
\begin{enumerate}
    \item 2-meter dewpoint temperature (d2m) (18.7\% importance)
    \item 2-meter temperature (t2m) (18.0\%)
    \item Vertical moisture gradient (7.6\%)
    \item Solar zenith angle (sza\_deg) (7.2\%)
    \item Boundary layer height (blh) (6.2\%)
    \item Shadow angle (shadow\_angle\_deg) (5.6\%)
    \item Solar azimuth angle (saa\_deg) (5.0\%)
    \item Atmospheric stability index (4.2\%)
    \item Temperature inversion height (4.0\%)
    \item Surface pressure (sp) (3.9\%)
\end{enumerate}

These top 10 features contribute 80.4\% cumulative importance; the remaining 8 features account for 19.6\%.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{paperfigures/figure3_feature_importance.png}
\caption{SHAP feature importance analysis for GBDT model. Near-surface moisture and temperature features (d2m, t2m) dominate predictions, along with atmospheric stability indicators and solar geometry features.}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Coverage:} 77.1\% (target: 90\%) → under-calibrated (intervals too narrow)
    \item \textbf{Mean interval width:} 533.4 m
    \item \textbf{Uncertainty-error correlation:} 0.485 (Spearman)
\end{itemize}

Figure~\ref{fig:uncertainty} shows that prediction intervals are informative despite under-calibration. The 77\% < 90\% coverage indicates intervals are too narrow to achieve nominal coverage, likely due to insufficient modeling of epistemic uncertainty. However, the positive correlation (r=0.485) between interval width and absolute error demonstrates that wider intervals successfully flag less reliable predictions. Post-hoc calibration using conformal prediction \cite{Shafer2008} could improve coverage while preserving this informativeness.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{paperfigures/figure4_uncertainty_quantification.png}
...
\begin{itemize}[leftmargin=*]
    ...
    \item Potentially different cloud regimes or meteorological conditions
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Atmospheric features dominate:} GBDT models using ERA5 reanalysis achieve R$^2$ = 0.744 (MAE = 117 m), outperforming CNNs on imagery by 2.0× in MAE.
