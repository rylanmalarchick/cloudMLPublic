...
From ERA5 reanalysis data and cloud shadow analysis, we engineer 18 features capturing atmospheric stability, moisture availability, and geometric properties:

\begin{enumerate}[leftmargin=*]
    \item \textbf{Atmospheric features from ERA5 (12):} 
    \begin{itemize}
        \item Boundary layer height (m)
        \item Lifting condensation level (LCL) height (m)
        \item Temperature inversion height (m)
        \item Vertical moisture gradient
        \item Atmospheric stability index
        \item 2-meter temperature and dewpoint (K)
        \item Surface pressure (Pa)
        \item Total column water vapor (kg/m$^2$)
        \item Solar zenith angle (degrees)
    \end{itemize}

    \item \textbf{Geometric features from shadow analysis (6):}
    \begin{itemize}
        \item Cloud edge coordinates (x, y pixels)
        \item Shadow edge coordinates (x, y pixels)
        \item Shadow length (pixels)
        \item Shadow detection confidence [0-1]
        \item Solar azimuth angle (degrees)
        \item Shadow angle (degrees)
    \end{itemize}
\end{enumerate}
